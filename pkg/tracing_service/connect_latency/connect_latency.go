...
func EnrichWithConnectLatencyEvent(base *schema.Base, event *tracing_service.BpfConnectLatencyEvent) error {
	...
	base.Event.Duration = tracing.ClampInt64(event.DurationNs)
	...
	base.Message = fmt.Sprintf(
		...
		srcAddr,
		dstAddr,
		time.Duration(tracing.ClampInt64(event.DurationNs)),
	)
	...
	return func(yield func(*tracing_service.EventResult, error) bool) {
		...
		err := tracing.RunTracingMapReceiver(
			...
			func(event *tracing_service.BpfConnectLatencyEvent) {
				...
				if err := EnrichWithConnectLatencyEvent(base, event); err != nil {
					mu.Lock()
					defer mu.Unlock()
					yield(nil, fmt.Errorf("enrich with connect latency event: %w", err))
					cancelReceiver()
					return
				}
