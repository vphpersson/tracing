...
func TestEnrichGuards(t *testing.T) {
	...
	testCases := []struct {
		name  string
		base  *schema.Base
		event *tracing_service.BpfDestroyConnectionEvent
	}{
		{name: "nil base", base: nil, event: &tracing_service.BpfDestroyConnectionEvent{}},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err := EnrichWithDestroyConnectionEvent(testCase.base, testCase.event)
			if err != nil {
...
func TestEnrichPopulatesTheDocument(t *testing.T) {
	...
	event := &tracing_service.BpfDestroyConnectionEvent{TimestampNs: 1_000_000_000, SourceAddress: ipv4Address(192, 0, 2, 1), DestinationAddress: ipv4Address(198, 51, 100, 2), SourcePort: 54321, DestinationPort: 443, AddressFamily: 2, TransportProtocol: 6, TcpState: 8, Start: 1_000_000_000, Stop: 2_000_000_000}

	base := &schema.Base{}
	_, err := EnrichWithDestroyConnectionEvent(base, event)
	if err != nil {
