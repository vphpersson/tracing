...
// IANA protocol numbers for the transports conntrack reports on.
const (
	protocolNumberTcp = 6
...
func EnrichWithDestroyConnectionEvent(base *schema.Base, event *tracing_service.BpfDestroyConnectionEvent) ([]any, error) {
	...
	if event.Start != 0 || event.Stop != 0 {
		...
		ecsEvent.Start = time.Unix(
			tracing.ClampInt64(event.Start/1e9),
			tracing.ClampInt64(event.Start%1e9),
		).UTC().Format("2006-01-02T15:04:05.999999999Z")

		ecsEvent.End = time.Unix(
			tracing.ClampInt64(event.Stop/1e9),
			tracing.ClampInt64(event.Stop%1e9),
		).UTC().Format("2006-01-02T15:04:05.999999999Z")

		if event.Stop > event.Start {
			ecsEvent.Duration = tracing.ClampInt64(event.Stop - event.Start)
		}
	}

	tcpStateName, ok := tcpStateIdToName[event.TcpState]
	if event.TransportProtocol == protocolNumberTcp && ok {
		ecsTcp := base.Tcp
		if ecsTcp == nil {
			ecsTcp = &schema.Tcp{}
			base.Tcp = ecsTcp
	...
	if event.SourceBytes != 0 || event.SourcePackets != 0 || event.DestinationBytes != 0 || event.DestinationPackets != 0 {
		if base.Source != nil {
			base.Source.Bytes = tracing.ClampInt(event.SourceBytes)
			base.Source.Packets = tracing.ClampInt(event.SourcePackets)
		}
		if base.Destination != nil {
			base.Destination.Bytes = tracing.ClampInt(event.DestinationBytes)
			base.Destination.Packets = tracing.ClampInt(event.DestinationPackets)
		}
		if base.Network == nil {
			base.Network = &schema.Network{}
		}
		base.Network.Bytes = tracing.ClampInt64(event.SourceBytes) + tracing.ClampInt64(event.DestinationBytes)
		base.Network.Packets = tracing.ClampInt64(event.SourcePackets) + tracing.ClampInt64(event.DestinationPackets)
	}

	var isError bool
	var errorMessage string
	switch event.TransportProtocol {
	...
	if event.TransportProtocol == protocolNumberTcp {
...
func Run(ctx context.Context, program *ebpf.Program, ebpfMap *ebpf.Map) iter.Seq2[*tracing_service.EventResult, error] {
	return func(yield func(*tracing_service.EventResult, error) bool) {
		if program == nil {
			yield(nil, altshiftErrors.NewWithTrace(nil_error.New("program")))
			return
		}

		if ebpfMap == nil {
			yield(nil, altshiftErrors.NewWithTrace(nil_error.New("ebpf map")))
			return
		}
		...
		err := tracing.RunTracingMapReceiver(
			...
			func(event *tracing_service.BpfDestroyConnectionEvent) {
				...
				tracingArgs, err := EnrichWithDestroyConnectionEvent(base, event)
				if err != nil {
					mu.Lock()
					defer mu.Unlock()
					yield(nil, fmt.Errorf("enrich with destroy connection event: %w", err))
					cancelReceiver()
					return
				}
