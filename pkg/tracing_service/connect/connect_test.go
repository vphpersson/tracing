...
func TestEnrichGuards(t *testing.T) {
	...
	testCases := []struct {
		name  string
		base  *schema.Base
		event *tracing_service.BpfConnectEvent
	}{
		{name: "nil base", base: nil, event: &tracing_service.BpfConnectEvent{}},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := EnrichWithConnectEvent(testCase.base, testCase.event)
			if err != nil {
...
func TestEnrichPopulatesTheDocument(t *testing.T) {
	...
	event := &tracing_service.BpfConnectEvent{TimestampNs: 1_000_000_000, SourceAddress: ipv4Address(192, 0, 2, 1), DestinationAddress: ipv4Address(198, 51, 100, 2), SourcePort: 54321, DestinationPort: 443, AddressFamily: 2, TransportProtocol: 6, ProcessId: 4242}

	base := &schema.Base{}
	err := EnrichWithConnectEvent(base, event)
	if err != nil {
