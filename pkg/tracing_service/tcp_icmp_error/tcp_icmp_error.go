...
// The errno names the kernel will set as sk_err for a given ICMP error.
const (
...
func classify(family uint16, icmpType uint8, icmpCode uint8) (string, string, string) {
	...
	case uint16(syscall.AF_INET):
		...
		if icmpType == 3 {
			...
			switch icmpCode {
			case 0:
				errnoCode = errnoNetUnreach
			case 1, 5, 6, 7, 11, 12:
				errnoCode = errnoHostUnreach
			case 2, 3:
				errnoCode = errnoConnRefused
			case 4:
				errnoCode = errnoMsgSize
			case 9, 10, 13:
				errnoCode = errnoHostUnreach
			}
		}
...
func Run(
	...
	return func(yield func(*tracing_service.EventResult, error) bool) {
		if programV4 == nil {
			yield(nil, altshiftErrors.NewWithTrace(nil_error.New("programV4")))
			return
		}

		if programV6 == nil {
			yield(nil, altshiftErrors.NewWithTrace(nil_error.New("programV6")))
			return
		}
		...
		if err != nil {
			yield(nil, altshiftErrors.NewWithTrace(fmt.Errorf("link attach tracing (v4): %w", err)))
			return
		}
		defer func() {
			if err := linkV4.Close(); err != nil {
				slog.WarnContext(
					altshiftContext.WithError(
						ctx,
						altshiftErrors.NewWithTrace(fmt.Errorf("tracing link close (v4): %w", err)),
					),
		...
		if err != nil {
			yield(nil, altshiftErrors.NewWithTrace(fmt.Errorf("link attach tracing (v6): %w", err)))
			return
		}
		defer func() {
			if err := linkV6.Close(); err != nil {
				slog.WarnContext(
					altshiftContext.WithError(
						ctx,
						altshiftErrors.NewWithTrace(fmt.Errorf("tracing link close (v6): %w", err)),
					),
					"An error occurred when closing a tracing link.",
				)
			}
		...
		err = tracing.RunMapReceiver(
			...
			func(event *tracing_service.BpfTcpIcmpErrorEvent) {
				...
				if err := EnrichWithTcpIcmpErrorEvent(base, event); err != nil {
					mu.Lock()
					defer mu.Unlock()
					yield(nil, fmt.Errorf("enrich with tcp icmp error event: %w", err))
					cancelReceiver()
					return
				}
