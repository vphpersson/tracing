...
func TestEnrichGuards(t *testing.T) {
	...
	testCases := []struct {
		name  string
		base  *schema.Base
		event *tracing_service.BpfTcpResetEvent
	}{
		{name: "nil base", base: nil, event: &tracing_service.BpfTcpResetEvent{}},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := EnrichWithTcpResetEvent(testCase.base, testCase.event)
			if err != nil {
...
func TestEnrichPopulatesTheDocument(t *testing.T) {
	...
	event := &tracing_service.BpfTcpResetEvent{TimestampNs: 1_000_000_000, SourceAddress: ipv4Address(192, 0, 2, 1), DestinationAddress: ipv4Address(198, 51, 100, 2), SourcePort: 54321, DestinationPort: 443, AddressFamily: 2, State: 1}

	base := &schema.Base{}
	err := EnrichWithTcpResetEvent(base, event)
	if err != nil {
