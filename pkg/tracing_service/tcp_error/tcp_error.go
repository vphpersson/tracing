...
func errnoName(err int32) string {
	// sk_err always holds a positive errno; anything else is not one to name.
	if err <= 0 {
	...
	case syscall.ECONNABORTED:
		return "ECONNABORTED"
	default:
		// Any other errno is reported by number rather than guessed at.
		return strconv.Itoa(int(err))
	}
}
	...
	return func(yield func(*tracing_service.EventResult, error) bool) {
		...
		err := tracing.RunTracingMapReceiver(
			...
			func(event *tracing_service.BpfTcpErrorEvent) {
				...
				if err := EnrichWithTcpErrorEvent(base, event); err != nil {
					mu.Lock()
					defer mu.Unlock()
					yield(nil, fmt.Errorf("enrich with tcp error event: %w", err))
					cancelReceiver()
					return
				}
